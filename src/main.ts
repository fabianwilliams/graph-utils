...
server.tool(
  ...
  {
    date: z.string().describe("ISO 8601 Date String (e.g. new Date().toISOString())"),
  },
  async ({ date }) => {
...
server.tool(
  ...
  "Construct a Microsoft Graph API URL from path and query parameters",
  {
    ...
    queryParams: z.record(z.string()).optional().describe("Optional query parameters like $filter, $select"),
  },
